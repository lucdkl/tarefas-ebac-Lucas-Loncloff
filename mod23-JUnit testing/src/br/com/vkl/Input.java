...
public class Input {
    ...
    //Usado para testes
    public Set<SeresZumanos> getGeneroTesting(String genero){
        String generoNoCase = genero.toLowerCase();
        Set<SeresZumanos> setGenEspecifico = lista.stream().filter(sz -> sz.getGenero().equalsIgnoreCase(generoNoCase)).collect(Collectors.toSet());
        return setGenEspecifico;
    }

    //Usado para testes
    public void addLista(SeresZumanos zumano){
        lista.add(zumano);
    }
}
